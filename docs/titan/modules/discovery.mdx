...
`forRoot()` accepts `DiscoveryModuleOptions`, which extends `DiscoveryOptions`
(the heartbeat / pub-sub fields) with the three module-wiring fields
(`redisUrl`, `redisOptions`, `enableNetronIntegration`).
