...
type ConfigSource =
  ...
  | { type: 'remote'; url, headers?, timeout?, retry? };   // `retry` is a number (attempt count)
```
...
| Class                       | Purpose                                                |
| --------------------------- | ------------------------------------------------------ |
| `ConfigService`             | Main API — `get`, `set`, `has`, `getAll`, `getTyped`, `onChange`, `reload`, `validate`, … |
...
The loader retries automatically on transient failures:
- Up to **3 attempts** total per remote source.
- Exponential backoff with ±30% jitter (200 ms base, capped at 2 s).
- Only **retryable** failures are retried (5xx, 408/429, network
  errnos, request-timeout `AbortError`); a 4xx or a 2xx-with-bad-body
  fast-fails without retrying.
- After exhaustion, throws — boot aborts.

> The retry policy (attempt count and backoff) is **fixed in the
> loader**, not configurable per-source. `IRemoteConfigSource` carries
> an optional `retry?: number` field, but the current loader does not
> read it.
...
`optional` means something narrower for a **file** source: the file may be
absent, but a file that exists and cannot be parsed or read still throws. See
[what `optional` forgives](../configuration/sources.md#what-optional-forgives).
