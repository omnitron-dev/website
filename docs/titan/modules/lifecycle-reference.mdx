...
`titan-health` is **lifecycle-passive**: the module registers its
indicators eagerly at config time (inside `forRoot` / `forFeature`),
not in a lifecycle hook. Checks are *pull-driven* — Kubernetes probes
invoke `check()` on demand. No background loop runs.
...
| `onModuleDestroy` | Disconnects all named clients gracefully (`closeAllClients()`) |

`titan-redis` uses the `onModuleInit` / `onModuleDestroy` hook names
(on `RedisManager`) — functionally equivalent to `onInit` / `onDestroy`
in newer modules. There is no `onStop` hook; cleanup runs in
`onModuleDestroy`.
...
sequenceDiagram
  ...
  App->>Disc: onDestroy
  App->>RL: destroy()
  App->>Redis: onModuleDestroy (disconnect)
