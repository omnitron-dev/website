...
| `@RateLimit`     | `@omnitron-dev/titan` (composable) + `titan-ratelimit` + `titan-pm`         | Use titan-ratelimit for service methods        |
| `@Cache`         | `@omnitron-dev/titan` (composable) + `titan-pm` (worker)                    | Compare with `@Cacheable` from titan-cache     |
