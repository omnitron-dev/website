...
Every entry below is checked against the packages' real exports by a
script, not by hand — a hand check is true on the day it is made and
decays silently afterwards:

```bash
node internal/website/scripts/check-titan-inventories.mjs .
```

It exits non-zero if this page names a decorator no package exports, or
if the set the root re-exports stops being exactly the nine listed
above.
...
| Auto-emit an event from a method              | `@EmitEvent({ event: 'user.created' })` *(titan-events)* |
| Wrap a method in a distributed lock           | `@WithDistributedLock(key, ttl)` *(titan-lock)*          |
| Instrument a method with RPC counters + latency | `@Metrics(metricName?)` *(titan-metrics)*               |
...
Imported from `@omnitron-dev/titan/decorators`, which exports all of
them. The package **root** re-exports exactly nine — `Service`,
`Injectable`, `Inject`, `Optional`, `Singleton`, `Transient`,
`Module`, `PostConstruct`, `PreDestroy` — and nothing else, so
`@Public`, `@Validate`, `@Timeout`, `@RateLimit`, `@Cache` and the
rest below are **not** importable from `@omnitron-dev/titan`. Real
code follows this: every one of the 52 `@Public` imports in the DAOS
backends reads `from '@omnitron-dev/titan/decorators'`.
...
Core also ships three lightweight **composable annotations** meant
to sit alongside `@Public()` — they write metadata that transports /
modules read, rather than implementing the behaviour themselves:

| Decorator        | Targets | Effect                                              |
| ---------------- | ------- | --------------------------------------------------- |
| `@Auth(opts?)`   | method  | Marks the method as requiring authentication        |
| `@RateLimit(opts?)` | method | Attaches rate-limit metadata to the method         |
| `@Cache(opts?)`  | method  | Attaches cache metadata to the method               |

> These share names with the satellite-package decorators
> (`titan-ratelimit.@RateLimit`, `titan-pm.@Cache`, etc.). The core
> ones are bare metadata annotations; see the collision table below.
...
> distinguish by import path (`@omnitron-dev/titan/decorators` vs
...
> Import path determines which: `@omnitron-dev/titan/decorators`
> (expose) vs `@omnitron-dev/titan-auth` (auth bypass).
...
| `@EmitEvent({ event, mapResult?, mapError? })` | method | Emits `<event>.success` after a return, `<event>.error` on a throw |
...
| Decorator                                                       | Targets | Effect                                                  |
| --------------------------------------------------------------- | ------- | ------------------------------------------------------- |
| `@Metrics(metricName?)`                                         | method  | Auto-track `rpc_requests_total`, `rpc_request_duration_seconds`, `rpc_errors_total` around the call (optional name prefix) |
...
| `@HealthCheck(opts?)`                    | method          | Marks a method as the process's health check                 |
| `@OnShutdown(opts?)`                     | method          | Runs the method during graceful worker shutdown              |
...
> A `@SelfHeal(action)` decorator is defined in the package but is
> **not** re-exported from the `@omnitron-dev/titan-pm` barrel — it
> isn't importable from the package root.
...
`titan-redis` ships only injection decorators — `@InjectRedis()`
for the default client and `@InjectRedisManager()` for the manager
(plus `@RedisCache` / `@RedisLock` / `@RedisRateLimit` helpers). For
token-based injection use `REDIS_MANAGER` or
`getRedisClientToken(name)`. See [`titan-redis`](./redis.mdx).
...
| ---------------- | --------------------------------------------------------------------------- | ---------------------------------------------- |
| `@Public`        | `@omnitron-dev/titan/decorators` (expose) + `titan-auth` (auth bypass) + `titan-pm` (worker IPC) | Pick by purpose; compose if needed     |
| `@Validate`      | `@omnitron-dev/titan/decorators` + `titan-pm`                                          | Core for Netron methods; pm for workers        |
| `@Timeout`       | `@omnitron-dev/titan/decorators` (abort) + `titan-scheduler` (one-shot)                | Read the verb — abort vs defer                 |
| `@RateLimit`     | `@omnitron-dev/titan/decorators` (composable) + `titan-ratelimit` + `titan-pm`         | Use titan-ratelimit for service methods        |
| `@Cache`         | `@omnitron-dev/titan/decorators` (composable) + `titan-pm` (worker)                    | Compare with `@Cacheable` from titan-cache     |
