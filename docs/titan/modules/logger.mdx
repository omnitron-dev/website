...
(`logger.service.ts:243-299`, `:24-45`):
...
`LoggerService.flush()` (`logger.service.ts:529-565`, `:587-589`):
...
> transport is registered then (`logger.service.ts:275-279`). So
...
to pino when it builds the root logger (`logger.service.ts:199`). It
...
(read at startup, `logger.service.ts:368`). Titan redacts nothing by
...
- **Transform or drop per line** → a `processors` entry. Each runs on
  ...
  to drop it (`logger.service.ts:464-517`).
