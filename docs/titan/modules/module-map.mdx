...
(`storageType: 'redis'`, `multiTier: true`, `enableDatabaseIndicator: true`).
...
| `titan-ratelimit`            | Standalone (`+ titan-redis` if `storageType: 'redis'`)    |
...
| Memoize a function call             | [`titan-cache`](./cache.mdx)                  | `@Cacheable(...)` decorator; multi-tier (L1+L2)            |
...
| `titan-cache`                | bare `titan-redis`                               | Caching semantics (`@Cacheable`, TTL, multi-tier)     |
...
`TitanRedisModule.forFeature(['cache', 'queue', …])` (or the
`clients: [...]` option on `forRoot`).
