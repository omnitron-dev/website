...
@EmitEvent({
  ...
  mapError:  (err)  => ({ reason: err.message }),
})
async create(input: CreateInput) { /* … */ }
```

The interceptor runs **after** the method: on success it emits
`<event>.success` (payload = `mapResult(result)` if provided, else the
raw result); on throw it emits `<event>.error` (payload =
`mapError(error)` if provided, else the error) and re-throws. Options
are `event`, `mapResult?`, `mapError?` only.

Two things about it surprise people, and both are silent.

**Subscribers must use the suffixed names.** `event: 'user.created'` never
emits `user.created`; subscribing to that bare name receives nothing.
...
neither it runs the method and emits nothing — no warning, no error. A service
that calls its emitter something else gets a decorator that does nothing at
all — and something else is the norm: all 24 services in the daos backends that
hold an emitter call it `events`. The property name is part of the contract.

### `@ScheduleEvent(options)`

```typescript
@ScheduleEvent({ event: 'user.welcome-followup', delay: 24 * 60 * 60 * 1000 })   // 24h delay
async welcomeFollowup() { /* … */ }

@ScheduleEvent({ event: 'reports.daily', cron: '0 9 * * *' })
async dailyReport() { /* … */ }
```

Options: `event` (required), plus one of `delay` (ms), `cron`, or
`at` (a `Date`).

### `@BatchEvents(options)`

Coalesce many emits into one delivery — useful when a tight loop
would otherwise fire thousands of identical events. Options: `event`,
`maxSize`, and `maxWait` (ms) are all required.

### `@OnModuleEvent({ event, filter? })` — listen to framework module events
