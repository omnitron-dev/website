...
† `SessionRevokedAuthError` and `InvalidTokenClaimError` are exported
from the `@omnitron-dev/titan-auth/preset/shared-session` subpath, not
the package root. The other three are root exports.
...
> `PoolBackpressureError` is **not re-exported** from the
> `@omnitron-dev/titan-pm` barrel, so you can't `instanceof` it from
> the package root — narrow on `error.name === 'PoolBackpressureError'`
> instead.
...
try {
  ...
  if (e instanceof Error && e.name === 'PoolBackpressureError') {
    await sleep(100);
    return retry();
  }
  throw e;
...
Carries an `IRateLimitResult` on its `.result` property (the fields
are nested under `error.result`, not on the error directly):

| `error.result` field | Type    | Meaning                                     |
| -------------------- | ------- | ------------------------------------------- |
| `retryAfter`         | `number` (seconds) | Suggested back-off before next attempt |
...
`@omnitron-dev/titan/utils` ships pre-built
[retry](../resilience/retry.md) and
[circuit-breaker](../resilience/circuit-breaker.md) primitives
(`retry()`, `CircuitBreaker`, `withTimeout()`) that already encode
this matrix — wire one of those rather than hand-rolling.
